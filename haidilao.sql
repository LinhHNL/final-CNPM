...
CREATE PROCEDURE SELECTALLMONAN
AS
BEGIN
	SELECT * FROM MONAN
END
GO
/******  stored procedure INSERT<DBO.MONAN> ******/
CREATE PROCEDURE INSERTTOMONAN(@MAMON CHAR(10), @TEN NVARCHAR(45), @GIA FLOAT, @ANH TEXT, @DIEM INT, @MALOAI CHAR(10))
AS BEGIN
	INSERT INTO MONAN(MAMONAN, TEN, GIABAN, ANH, DIEM, MALOAIMONAN)
	VALUES(@MAMON, @TEN, @GIA, @ANH, @DIEM, @MALOAI)
END
GO
/******  stored procedure DELETE<DBO.MONAN> ******/
CREATE PROCEDURE DELETEMONAN(@MAMON CHAR(10))
AS 
BEGIN
	DELETE FROM MONAN WHERE MONAN.MAMONAN = @MAMON
END

/******  stored procedure UPDATE<DBO.MONAN> ******/
GO
CREATE PROCEDURE UPDATEMONAN(@MAMON CHAR(10), @TEN NVARCHAR(45), @GIA FLOAT, @ANH TEXT, @DIEM INT, @MALOAI CHAR(10))
AS 
BEGIN
	UPDATE MONAN
	SET MONAN.TEN=@TEN, MONAN.GIABAN=@GIA, MONAN.ANH=@ANH, MONAN.DIEM=@DIEM
	WHERE MONAN.MAMONAN=@MAMON
END